...
/*
	"Googler Web Font"-г татаж өөрийн сэрвэрт байршуулан ашиглах 
	Roboto Condensed тигийг жишээ болон ашигласан болно
*/
...
preg_match_all($reg_exUrl, $data, $match);



$hadgalahzam = dirname(__FILE__);

/* 
** Тигийн үндсэн замын хавтасыг тэмдэглэж авна
*/
$undsenzam = 'http://fonts.gstatic.com/s/robotocondensed/v13/';

/*
** Тигийг хадаглах хавтас 
*/
$hadgalahzam = '/style/fonts/';
...
foreach ($match[0] as $font) {
	$newFont = str_replace($undsenzam, $hadgalahzam, $font);
	file_put_contents( dirname(__FILE__) . $newFont, file_get_contents($font ));
}

$newData = str_replace($undsenzam, $domainname.$hadgalahzam, $data);


$file = fopen( dirname(__FILE__).$hadgalahzam."RobotoCondensed.css","w");
fwrite($file, $newData);
fclose($file);

echo dirname(__FILE__).$hadgalahzam."RobotoCondensed.css";
